...
class Select extends display.Container {
  ...
  init({ closedBG, textStyle, items, openBG, selected, selectedTextOffset, scrollBox, visibleItems }) {
    ...
    if (!this.openButton) {
      ...
      this.openButton.textView = new text.Text(items?.items ? items.items[0] : "", textStyle);
    ...
    if (!this.closeButton) {
      ...
      this.closeButton.defaultView = new graphics.Graphics().beginFill(0, 1e-5).drawRect(0, 0, this.openButton.width, this.openButton.height);
      this.closeButton.textView = new text.Text(items?.items ? items.items[0] : "", textStyle);
      this.openButton.textOffset = selectedTextOffset;
    }
    if (!this.scrollBox) {
